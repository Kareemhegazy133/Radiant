...
- **Crash-class bugs (Phase 1):** `DrawLine` has no capacity check (RAD-15); `OpenGLTexture2D::SetData` leaks a buffer copy per call (RAD-14). (RAD-10's Shutdown double-free is fixed: batch base arrays are `Scope<T[]>`-owned; cursors are raw non-owning by convention.)
