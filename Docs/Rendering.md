...
- **Crash-class bugs (Phase 1):** `Renderer2D::Shutdown` deletes batch *cursor* pointers as well as base allocations (RAD-10); `DrawLine` has no capacity check (RAD-15); `OpenGLTexture2D::SetData` leaks a buffer copy per call (RAD-14).
- **Legacy remnants:** `Renderer::Submit` (per-object, string-name uniforms) is incompatible with the 450-core shaders and dies with the GL backend; engine shaders currently live duplicated inside game asset folders (moved engine-side in Phase 4, RAD-48).
